...
class AnomalyDetector:
    """Detect grid instability using anomaly detection"""
    ...
    def get_instability_score(self, state: np.ndarray) -> float:
        """Get normalized instability score [0, 1]"""
        if not self.is_fitted:
            return 0.0
...
class InstabilityPredictor:
    """Predict future instability based on current trends"""
    ...
    def update(self, state: np.ndarray) -> Dict:
        """Update with new state and get warning"""
...
class EarlyWarningSystem:
    ...
    def get_lead_time_estimate(self) -> int:
        """Estimate minutes to potential failure"""
