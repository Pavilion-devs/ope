...
class GridGuardEnv(gym.Env):
    ...
    def __init__(self, max_steps: int = 100, use_mock: bool = True):
        ...
        if use_mock:
            ...
            from src.gridguard.environment import IEEE14BusGrid
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        obs = np.concatenate([
            state.bus_voltages,                      # 14
            state.phase_angles,                      # 14
            state.line_loadings[:20],                # 20
            state.generator_outputs[:5] / 200.0,     # 5
            [state.frequency_deviation]              # 1
        ])
