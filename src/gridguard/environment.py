...
class IEEE14BusGrid:
    ...
    def apply_disturbance(self, disturbance_type: str, magnitude: float = 1.0):
        """Apply a disturbance to the grid"""
    ...
    def apply_action(self, action: np.ndarray) -> float:
        ...
        if self.pf_converged:
            ...
            if state is not None:
                voltage_violation = np.sum(np.abs(state.bus_voltages - 1.0) > 0.1)
                loading_violation = np.sum(state.line_loadings > 0.9)
                reward = -(voltage_violation * 10 + loading_violation * 5)
...
class GridGuardEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        # Apply action (soft control)
        reward = self.grid.apply_action(action)
        ...
        # Bonus for stability
        if not terminated and state is not None:
